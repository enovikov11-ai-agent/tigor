  ...
  outputs = {
    ...
    gnomeModule = {
      ...
      environment.systemPackages =
        (with pkgs; [nautilus gnome-console])
    ...
    # vm selects platform plumbing; workload and hypervisor flags work on either platform.
    stateless = {
      vm ? false,
      gnome ? false,
      extras ? false,
          ...
          [
            ({
              ...
              networking = {
                ...
                nftables.enable = lib.mkIf (!vm || hypervisor) true;
              ...
              boot.supportedFilesystems = lib.optionals (!vm) ["zfs"];
              boot.initrd.kernelModules =
                lib.optionals vfio ["vfio_pci" "vfio" "vfio_iommu_type1"]
                ++ lib.optionals (!vm && gnome && !nvidia) ["nouveau"];
              boot.kernelModules =
                lib.optionals vm ["virtiofs"]
                ++ lib.optionals hypervisor ["kvm-amd"];
              boot.kernelParams = lib.optionals (!vm) (
              ...
              boot.blacklistedKernelModules = lib.optionals (!vm) ["ast"];
              boot.uki = {
                name = efiName;
                version = null;
                settings.UKI.Initrd = lib.mkForce "${config.system.build.netbootRamdisk}/initrd";
              };
              boot.zfs.forceImportRoot = lib.mkIf (!vm) false;
